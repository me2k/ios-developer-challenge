...
enum Endpoints: URLRequestConvertible {
    ...
    private var parameters: Parameters? {
        ...
        let hash = MD5(String(timeInterval) + APIConstants.apiPrivateKey + APIConstants.apiPublicKey).lowercased()
        var parameters: Parameters = [ "apikey" : APIConstants.apiPublicKey,
                                           "ts" : timeInterval,
                                           "hash" : hash]
        switch self {
    ...
    func asURLRequest() throws -> URLRequest {
        let url = try APIConstants.baseAPIUrl.asURL()
